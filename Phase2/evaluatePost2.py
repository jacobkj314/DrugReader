...
def evaluate():
    ...
    for i, doc in enumerate(test):
        ...
        if useGold:
            docWithEntities = [(sentenceText, drugChars, interactions) for sentenceText, drugChars, interactions in doc]
            extractedRelations = {(first, second, label) for first, second, label, _ in utilsPost.extractRelationsFromGoldEntities(docWithEntities)} #use if providing gold entites
        else:

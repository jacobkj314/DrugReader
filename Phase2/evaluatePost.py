...
def evaluate():
    ...
    for i, doc in enumerate(test):
        ...
        for sentenceText, drugChars, interactions in doc:
            ...
            interactions = [(drugs[first], drugs[second]) for first, second, label in interactions]
            for interaction in interactions:
                actualRelations.add(interaction)
        
        if useGold:
            #docWithEntities = [(sentenceText, drugChars) for sentenceText, drugChars, _ in doc]
            extractedRelations = {(first, second) for first, second, _, _ in utilsPost.extractRelationsFromGoldEntities(doc)} #use if providing gold entites
        else:
            docText = " ".join([sentenceText for sentenceText, _, _ in doc])#assemble document - use if not providing gold entities
            extractedRelations = {(first, second, label) for first, second, label, _ in utilsPost.extractRelations(docText)} #use if not providing gold entities
    ...
    print(f"S:{totalPrecision}\t\t{totalRecall}\t\t{totalF}")

if __name__ == "__main__":
    if "-test" in sys.argv:
        test = utilsPost.getGold("Test")

    if "-goldIgnore" in sys.argv:
        useGold = False

    if "-mbin" in sys.argv:
        utilsPost.detectRelation = utilsPost.detectRelationMultiBinary
        utilsPost.resolveCollisions = False
    elif "-mbinResolve" in sys.argv:
        utilsPost.detectRelation = utilsPost.detectRelationMultiBinary
    elif "-mclass" in sys.argv:
        utilsPost.detectRelation = utilsPost.detectRelationMulticlass
    elif "-pipe" in sys.argv:
        utilsPost.detectRelation = utilsPost.detectRelationPipeline

#This python script trains the multibinary classifiers on extracted context vectors using sklearn. Written by Jacob Johnson for CS6390 at the University of Utah, Spring 2022.
...
def main():
    ...
    golds: dict[str, list[ndarray]] = pickle.load(open("../vectors/goldVectors-peak", "rb"))#load best vectors from phase1
    ...
    for label in labelOptions:
        ...
        negatives = pickle.load(open("../vectors/negativeVectors-peak", "rb"))
        print(len(negatives))
        for vector in negatives[::(49514//((7 if label == "advise" else 8)*len(gold)//4))]:#get 1.75x as many negatives as positives for "advise", 2x as many for all other labels, as determined by devset experiments
            trainData = pd.concat([trainData, vector], ignore_index = True)#append
            labels.append("false")
        #"""
        ...
        pickle.dump(train, open(f"../models/{label}Classifier", "wb"))

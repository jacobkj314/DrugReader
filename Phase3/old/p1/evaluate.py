...
def evaluate(partition = pickle.load(open("Dev/DEV", "rb")), useGold = True, classifier =None):
    ...
    for i, doc in enumerate(partition):
        ...
        for sentenceText, drugChars, interactions in doc:
            ...
            interactions = [(MentionPair(drugs[first], drugs[second]), label) for first, second, label in interactions]
            for interaction in interactions:
                actualRelations.add(interaction)

        if useGold:
            docWithEntities = [(sentenceText, drugChars) for sentenceText, drugChars, _ in doc]
            extractedRelations = {(MentionPair(first, second), label) for first, second, label, sentence in utils.extractRelationsFromGoldEntities(docWithEntities)} #use if providing gold entites
        else:
...
def main():
    ...
    if True:# #"-test" in sys.argv:
        ...
        for fold in range(5):
            ...
            results.append(evaluate(classifier = None, partition = test))#evaluate on test partition
